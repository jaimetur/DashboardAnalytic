...
7. Use **Generate Report Charts** to queue every automated template chart, or generate the PowerPoint report. Both continue on the server after leaving Reporting or signing out. **Generated Reports Jobs** contains only PowerPoint reports; the separate **Generated Report Charts Jobs** panel shows Chart Set progress, failures, retries and ready sets that can be opened in Report Charts. New PPTX files are stored under the active workspace's `output/reports` directory, while older jobs remain accessible from the former `exports` location. Each job stores its selected datasets, technology, scope and Slides Template in SQLite for auditability.
