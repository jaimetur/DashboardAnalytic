...
7. Generate the PowerPoint report. It is queued as a persistent background job, so it continues after leaving Reporting or signing out. **Generated Report Jobs** shows its ID, date, report name, selected datasets, slide count, status and progress; ready reports can be downloaded, opened or deleted. The run stores its selected datasets, technology, scope and Slides Template in SQLite for auditability.
