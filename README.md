...
7. Generate the PowerPoint report. It is queued as a persistent background job, so it continues after leaving Reporting or signing out. **Generated Reports Jobs** shows the complete persisted history with ID, date, report name, selected datasets, slide count, status and progress; ready reports can be downloaded, opened or deleted. New PPTX files are stored under the active workspace's `output/reports` directory, while older jobs remain accessible from the former `exports` location. The run stores its selected datasets, technology, scope and Slides Template in SQLite for auditability.
