...
When a report combines historical campaigns, reporting normalises known UK operator aliases in memory before catalogue filters and groupings run. For example, `Vodafone`/`Vodafone UK`, `O2(UK)`/`o2 - de`, `3`/`Three`/`three(uk)`, and EE variants are consolidated into one operator label. This report-only step also makes `Operator IN (Vodafone, O2, 3, EE)` match the corresponding historical spellings; it never changes the stored CDR data.
