...
- Added compact pastel fixed left-side Slide Catalogue Editor row controls (**↑**, **↓**, green **+**, **−**) that remain visible and centred during horizontal scrolling, with an unobtrusive opaque table-tone backing and a shared fixed-width anchor that prevents scrolled cells from bleeding below the actions or displacing **Slide**. A violet **Re-Enumerate Slides** action precedes **Save Catalogue** to stably order slide blocks and renumber their distinct values consecutively without altering their chart content; a mouse-wheel/Shift+wheel scrolling hint follows Save. The editor uses vertically centred cells, with expanded **Slide tittle**, **Slide Subtittle**, **Chart Tittle**, **Chart type** and **Filters** columns. Row order directly controls the generated placeholder order for charts sharing a slide.
