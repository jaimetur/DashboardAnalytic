...
- Standardised Vendor Comparison charts by automatically excluding Mixed and Other vendors, resolving template Operator filters from the materialised operator prefix, preserving full vendor identities in aggregations and legends, applying standard aliases and matching vendor-family colours, and ordering mapped vendors consistently while retaining chronological Campaign values.
...
- Reworked Chart Preview with a taller desktop dialog and a viewport-fitted mobile-landscape canvas: smaller, thicker chart navigation and header actions preserve the portrait layout; magnifying-glass zoom controls and boundary-aware overlaid pan arrows move the viewport in their labelled direction; responsive hover tooltips use the renderer's exact CDF geometry and series identity to identify the line under the pointer, and cover horizontal as well as vertical stacked-bar segments with their labels, values and legends after chart changes, zooming or panning; and the canvas repaints after mobile rotation while preserving zoom.
