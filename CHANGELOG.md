...
- Split Slides Templates Editor controls into non-fixed per-chart actions and block-level Slide Actions, with the move, insert and delete controls consistently grouped on the first action row, constrained chart reordering, whole-slide movement/insertion/deletion and correct renumbering; added in-place duplication plus chart/slide copying to any destination template through one dialog, showing only Slide position for slide copies and Destination slide plus Chart position for chart copies; selected dropdown values stay at the top and unsaved values and edited-cell highlighting survive table rebuilds. Added workspace-owned Calculated Dimensions management from Workspace, Slides Templates Editor and Chart Preview through a wider responsive dialog that displays every numbered rule and provides add, edit, delete and independent JSON import/export controls to workspace users; former per-template JSON sidecars are migrated and removed, while definitions such as Result Group, Call Family and Test Family are materialised in the background at the end of every applicable individual and combined CDR table, highlighted in CDR Preview and exposed in every applicable dataset-field selector without delaying Workspace rendering. The management and slide re-enumeration buttons use the shared yellow gradient.
