...
#### 🚀 Enhancements:
- Reused shared materialized CDR tables and Reporting chart preparation for Dashboards, with consistent filtered snapshots across slides, debounced filter changes and stale-response protection. Dashboard opening now compiles datasets, dates, NR Mode and adaptive filters into indexed SQLite predicates, aggregates compatible filter facets in one scan, persists facet values and row counts, materializes compact row-key selections only up to 25,000 rows and queries larger selections directly without delaying the viewer with large key inserts. Charts join that shared selection to the combined CDR table and load only their KPI, grouping and template-filter columns; bounded workspace PNG caching makes previously rendered charts reusable after restart. Added incremental shared-CDR column backfills instead of recreating complete tables, adaptive cache invalidation from dataset and combined-table revisions, immediate first-dashboard chart prefetch plus following-dashboard prefetch, concurrent slide rendering, a distinct muted-violet dashboard theme and responsive chart layouts. A prominent pastel-yellow loading notice keeps the unavailable state clear while preparation runs, then changes to pastel green when View Dashboard is available.
