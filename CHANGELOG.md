...
- Added conditional **Map Vendors** actions for unassigned CDRs in Workspace. The dialog lists only ready VFUK and 3UK mappings available in that Workspace and persistently assigns each sample Vendor from its Operator plus the first/last `Cell_ID_A`, using the agreed Vodafone/Three formula; compact CDR actions share one row, with Map Vendors last, a dedicated mapping colour and a narrower Progress column.
