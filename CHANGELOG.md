...
- Allowed PowerPoint Report and Chart Set jobs to run from any non-empty combination of Data, Voice and Speech CDRs, rendering an explicit unavailable-source placeholder for chart types without a selected CDR; added synchronized Tech, Type, Template and Scope filters to the Charts Panel selector and combined Jobs table.
- Split Slides Templates Editor controls into non-fixed per-chart actions and block-level Slide Actions, with constrained chart reordering, whole-slide movement/insertion/deletion and correct renumbering; added in-place duplication plus chart/slide copying to any destination template through one dialog with dependent destination and position fields, and preserved unsaved values and edited-cell highlighting while rebuilding the table.
