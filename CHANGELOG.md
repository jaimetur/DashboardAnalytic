...
#### 🚀 Enhancements:
- Reused shared materialized CDR tables and Reporting chart preparation for Dashboards, with consistent filtered snapshots across slides, debounced filter changes, stale-response protection and bounded preview caches. Added incremental shared-CDR column backfills instead of recreating complete tables, normalized each filtered Dashboard snapshot once for all its charts, rendered slide chart requests concurrently and prefetched the following slide; added a distinct muted-violet dashboard theme and responsive chart layouts.
