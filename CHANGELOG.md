...
- Removed persistent Report Template compatibility directories after their one-time legacy CSV migration. Report Templates now remain database-backed at runtime and Backup/Export writes their CSV representation directly into the ZIP without recreating workspace or configuration template folders.
- Added a compact persistent minimize/expand control to each floating background-task card.
