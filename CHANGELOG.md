...
- Improved Workspace data access with an alphabetical **All Types** queue filter, disabled unavailable types, ordered **View**/**Show Dashboard** actions, two-axis preview scrolling and configurable preview rows (100 by default); VFUK previews now expose only selectable `4G`/`5G` sheets and their materialized GCID values, while both VFUK and 3UK mapping previews show source mapping columns rather than empty normalization fields. Older VFUK mappings are upgraded automatically when opened.
