...
#### 🚀 Enhancements:
- Reworked template-backed CDR rendering into an explicit NSA/SA slide contract: each automated slide now selects its CDR source, technology/session/test/direction filters, KPI and template chart grammar (100% stacked columns, failure counts, CDF-plus-summary or scatter) rather than reusing one generic bar chart. The complete catalog now records CDR source, KPI, chart type, filters and grouping per chart (one row per chart), with matching editable NSA/SA CSV exports.
- Added Admin export/import for NSA and SA Slide Catalogue CSVs stored in `assets/ppt-slides-catalog/`. The active catalogues become the rendering reference, refresh the PowerPoint Reporting Slide Catalogue documentation, reproduce each template slide's `Slide tittle`/`Slide Subtittle` headers and select the matching one- to four-image master layout according to the number of chart rows per slide.
