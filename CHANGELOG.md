...
#### 🚀 Enhancements:
- Reused shared materialized CDR tables and Reporting chart preparation for Dashboards, with consistent filtered snapshots, debounced changes and stale-response protection. Dashboard opening now loads filter catalogues from persisted dataset profiles and Auto-calculated Field rule domains instead of scanning wide combined CDR tables; small selections retain exact indexed row keys, while large selections become available immediately with source-row totals. A bounded, revision-keyed analytical SQLite cache stores only the fields required by each Dashboard/template, warms in the background on workspace activation and Dashboard save, and is rebuilt only when its datasets or required columns change. Each live chart reads its minimal projection, returns a reduced JSON model with bounded CDF/scatter samples or aggregated bars/tables, and is drawn responsively by the browser instead of waiting for a PNG; exact chart models persist across restarts, the visible slide is prioritised and later slides warm sequentially. Legacy PNG previews remain available for compatibility and export. Added incremental shared-CDR column backfills, deterministic invalidation, a distinct muted-violet theme, responsive template layouts, and a pastel-yellow preparation notice that turns pastel green when View Dashboard is available.
...
- Restricted E2E Dashboards to super-admins and the EJAITUR account, matched its inactive tab styling to the other E2E module tabs while retaining its distinct violet active state, and expanded the dashboard viewer to a 96%-viewport 16:9 slide canvas so template chart layouts use the available screen area. Dashboard management now selects NR Mode beside its name and filters the Template list accordingly; its library is a collapsible table with a high-visibility total badge and compact white folder-open, white exit-door, duplicate, export and delete actions per dashboard. Import Dashboard clearly restores a previously exported Dashboard. The collapsible Dashboard Data panel identifies the open Dashboard, keeps Reset Filters beside Refresh Data, and uses floating Dataset Analysis-style multiselects split into Default Filters and a distinct pastel-pink Additional Filters subpanel; its additional-filter picker includes every selected-CDR column and Auto-calculated Field, uses the same-width searchable in-menu picker as the filters, and loads each added field on demand. Each filter has a confirmed circular removal action that also hides default filters until restored. Save persists the current filters as that Dashboard's defaults. Dashboard terminology, persistence and export format now use `dashboard` consistently, while existing stored configurations and export files migrate on use.
