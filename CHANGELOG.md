...
- Split Slides Templates Editor controls into non-fixed per-chart actions and block-level Slide Actions, with move, insert and delete controls consistently grouped on the first action row, constrained chart reordering, whole-slide movement/insertion/deletion, in-place duplication and chart/slide copying to a selected destination; selected dropdown values stay at the top and unsaved values and edited-cell highlighting survive table rebuilds. Refined workspace-owned Auto-calculated Fields management from Workspace, Slides Templates Editor and Chart Preview: the Chart Preview action now sits immediately beside Close, its wider dialog aligns its fields, compacts redundant case-only field aliases, documents case-insensitive matching, and uses a compact multi-choice Available for popover that closes after pointer exit or an outside interaction; the manager separates its Add action from the definition list, removes the editor Cancel action, provides a grey Close action beside Add or Save, keeps delete confirmations above the manager, replaces the minus deletion glyph with a cross, and adds distinct duplicate and purple download actions; every save requires confirmation before rematerializing applicable CDR tables, and a rename also updates every Slides Template reference to that field. Workspace places equal-width import and bounded, scrollable field-list subpanels after Datasets, labels the Field and Applied to columns, and keeps matching Manage and green Export All Auto-calculated Fields actions above the list. Definitions such as Result Group, Call Family and Test Family are materialised only in their applicable individual and combined CDR tables, with stale out-of-scope calculated columns removed during rematerialization, highlighted pastel red in CDR Preview and exposed only in applicable dataset-field selectors without delaying Workspace rendering; former per-template JSON sidecars are migrated and removed. Saving, editing, deleting and JSON importing now return immediately while a per-workspace background job applies incremental in-place SQLite column updates only to affected CDR types, normally scans each affected table once instead of loading and replacing it through pandas, reports table-level progress in the Workspace panel and notifies the browser on completion; identical imports are merged by normalized field name without duplicate definitions or materialized columns. Auto-calculated Fields can also be exported, imported and transferred as portable ZIP packages through Export / Import, with explicit multi-workspace destination selection on manual import and incoming transfer acceptance. Workspace metric selectors keep known empty metrics visible but disabled, workspace duplication keeps its asynchronous completion consistent, and imported database paths are relocated for both direct moves and cross-volume copies. The management and slide re-enumeration buttons use the shared yellow gradient.
