...
- Saving, editing, deleting, JSON importing and full rematerialization now return immediately while a per-workspace background job applies incremental in-place SQLite column updates only to affected CDR types, normally scans each affected table once instead of loading and replacing it through pandas, handles applicable tables without the source columns of a rule by using the field fallback, reports table-level progress in the Workspace panel and notifies the browser on completion. Identical imports are merged by normalized field name without duplicate definitions or materialized columns.
