...
- Reworked the compact Chart Preview into a viewport-fitted landscape canvas with smaller, thicker chart navigation and header actions for the filtered dataset and Interactive panel, while retaining the portrait layout; added magnifying-glass zoom controls and boundary-aware overlaid pan arrows in all four directions, and repaints the canvas correctly after mobile rotation while preserving zoom.
...
- Made App Logs filters faceted: each selector derives its values, including dates and event types, from every loaded row that matches the other active filters, including cards hidden by compact-phone pagination, and refreshes with live log data.
