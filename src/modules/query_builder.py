...
def execute_query(
...
) -> tuple[list[str], list[tuple[Any, ...]], bool, list[str]]:
    ...
    try:
        ...
        cursor = connection.execute(f'SELECT * FROM ({query}) LIMIT ?', (max(1, int(row_limit)) + 1,))
        rows = cursor.fetchmany(max(1, int(row_limit)) + 1)
        columns = [str(item[0]) for item in cursor.description or []]
        truncated = len(rows) > row_limit
        return columns, [tuple(row) for row in rows[:row_limit]], truncated, views
    finally:
        connection.close()
