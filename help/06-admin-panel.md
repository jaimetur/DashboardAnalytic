...
- **Workspace: _name_** exports one workspace database, uploaded input files and generated exports. Importing it creates a new local workspace; an existing workspace is never overwritten.
- **Full Environment** exports configuration, shared templates and every workspace. Importing it recreates the destination workspace registry from the imported workspaces, avoiding invalid absolute paths from the source deployment.

Use ZIP packages generated by Dashboard Analytic only. Package paths and manifests are validated before import.
