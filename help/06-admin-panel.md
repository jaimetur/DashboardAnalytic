...
Template CSV files are the shared source of truth. Template names, type and default state are indexed in `config/application.db`, independently of the active workspace, without a JSON registry or slug aliases. User accounts are stored in the same global application database.

Import a UTF-8 CSV by giving it a name and selecting the target NSA or SA section. The current schema is `Slide`, `Slide Tittle`, `Slide Subtittle`, `Layout`, `Chart Tittle`, `CDR source`, `KPI`, `Chart type`, `Filters`, `Rows Aggregation`, `Column Aggregation`, `Legend` and `Legend Position`. `Legend Position` accepts `Top`, `Bottom`, `Left` or `Right`; it draws a horizontal legend at the top/bottom or a vertical legend at the left/right. Blank values default to `Top`. Each distinct slide number creates a new slide from the named template layout; each CDR-source row represents one chart image. `Title Slide` and `Transition Slide` create structural slides and leave all KPI/chart fields empty. If an older compatible schema is supplied, the application asks in a floating dialog whether it should convert it. Conversion maps compatible headings, splits a former `Grouping` hierarchy into rows and columns, assigns missing layouts and migrates former preserve rows into structural slides. Validation failures are shown in a floating dialog and are also recorded in the audit log.

## Slides Templates Editor

Choose a stored template in the editor picker, then select **Edit**. The editable grid has horizontal and vertical scrolling. Every cell can be typed manually; contextual assistance opens only for fields with selectable/configurable values:

- **Layout**, **CDR source**, **KPI** and **Chart type** offer single, searchable selections.
- Rows are always presented and saved in ascending **Slide** order. When several chart rows share a slide, the editor displays its shared **Slide Tittle**, **Slide Subtittle** and **Layout** once as merged cells; editing one applies the value to every CSV row in that slide block. A fixed first column keeps the **↑**, **↓**, **+**, **⌕** and **−** row controls visible while scrolling. **+** creates a new unsaved sibling row with the same slide metadata/layout, ready for another chart definition; **⌕** previews the selected chart's effective input rows from all ready CDRs of its source type, using the current unsaved filters and aggregations. It also exposes resolved dimensions such as `Rate Bucket`. The arrows change chart order within the slide, while **−** removes the row until the template is saved. The final remaining row cannot be deleted. **Re-Enumerate Slides** renumbers distinct slide values consecutively from 1 without changing any other cell content; review the result, then use **Save Template** to persist it.
- **Rows Aggregation**, **Column Aggregation** and **Legend** offer searchable multi-selection and preserve existing selected values without duplicating dimensions. **Legend Position** offers the four supported placements.
...
Structural fields such as **Slide**, **Slide Tittle**, **Slide Subtittle** and **Chart Tittle** do not open Cell Assistance.
...
The **App Logs** tab combines the former Workspace execution trail and Administration audit log for the active workspace. It records significant access, configuration, upload, processing and report-generation actions, showing up to 1,000 recent events. Filter entries by **User**, **Date**, **Type** (`Info` or `Error`) and **Action** when investigating an incident; selections persist after reloading the page until **Clear Filters** is used.
