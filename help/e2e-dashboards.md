...
The job renders the template as a PPTX, preserving slides, layouts, chart placeholder proportions, titles, legends and saved comments. On a title cover slide, its subtitle uses the report Scope and its lower area shows Regions and Cities in aligned accent colours. It writes the PPT plus chart PNG, tooltip and Canvas-model assets under `output/dashboards`. Folder and PPT names begin with `yyyymmdd_HHMMSS`, followed by the Dashboard name, Scope and selected Regions joined with ` + ` (or `All Regions` when every Region is selected). Operators, Vendors and Cities remain in the job Filters column but are omitted from filenames and folder names. Long names are shortened only when necessary to fit filesystem component limits.
