...
Workspace, Dashboard and Reporting are shown as application tabs directly above the content panel. Readme, Changelog, Help, App Logs and (for administrators) Admin share that row on the right; only sign-out remains in the top navigation.

## Workspace

Start in **Workspaces Management**. The selector defaults to the most recently opened workspace. Use **Open** or **Close** to control the active workspace; **Manage Workspace** creates, renames, duplicates or removes workspaces. A closed workspace disables Dashboard and Reporting and hides ingestion/queue operations. Each workspace stores its own database, uploaded files and exports; Slides Templates are shared application configuration.
