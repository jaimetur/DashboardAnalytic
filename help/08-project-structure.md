...
- **Portable transfer:** Admin Import/Export writes validated ZIP packages for configuration, configuration with templates, an individual workspace or the full environment. Packages are built in `data/transfer-packages/` as temporary disk-backed jobs and are cleaned up after 24 hours. Workspace registries are intentionally local and regenerated on full-environment import.
