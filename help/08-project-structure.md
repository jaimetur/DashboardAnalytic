...
- **Workspaces and persistence:** `data/workspaces/workspace-registry.db` stores only the local workspace registry and active-state information. Each workspace lives at `data/workspaces/<Workspace Name>/`, with a same-named SQLite database plus `input/` and `output/reports/` directories. Dataset records, individual materialised rows and shared reporting rows are isolated in that database.
...
- **Portable transfer:** Admin Export / Import writes validated ZIP packages for configuration, configuration with templates, an individual workspace or the full environment, and can stream the selection to an approved destination server. Packages are built in `data/transfer-packages/` as temporary disk-backed jobs and are cleaned up after 24 hours. Workspace registries are intentionally local and regenerated on full-environment import.
